...
# Solo en get_slice con éxito y modo raw: cabecera Content-Length, línea vacía, N bytes de cuerpo.
...
def valid_status(s: int) -> bool:
    return s in error_messages


def fatal_status(s: int) -> bool:
    assert valid_status(s)
    return 100 <= s < 200


VALID_CHARS: set[str] = set(string.ascii_letters + string.digits + ".-_")

# Lista de comandos para la respuesta de "help" (uno por línea)
COMMANDS = [
    "get_file_listing",
    "get_metadata",
    "get_slice",
